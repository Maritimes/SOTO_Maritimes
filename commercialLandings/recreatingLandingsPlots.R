...
if (omitBenthInverts)landings<-landings[!landings$SPECIES_CATEGORY %in% "Benthic Invertebrates",]
library(dplyr)
all<- landings %>%
...
if (omitBenthInverts)all$SPECIES_CATEGORY <- factor(all$SPECIES_CATEGORY , levels=c("Pelagic Fish", "Demersal Fish") )
...
makeThePlot <- function(data = NULL, title = NULL){
  ...
  colors <- c("#7E62A1","#9BBB59","#C0514E")
  if (omitBenthInverts)colors <- c("#9BBB59","#C0514E")
